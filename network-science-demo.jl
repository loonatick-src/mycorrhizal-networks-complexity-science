...
# ╔═╡ 27359236-f152-11ec-2aae-db9fba9ad375
using NetworkDynamics, OrdinaryDiffEq, Plots, Graphs, LaTeXStrings

# ╔═╡ 548a5365-30d3-4c9d-a96a-957cd9572657
md"
# Network Diffusion Theory
Diffusion systems -- tend to evolve to a state of equally distributed heat, charge, or concentration. The scalar field changes according to the local gradient, typically proportional to the difference between its and neighbouring scalar values.

Consider graph $g(v, A)$, where $v=\begin{pmatrix} v_1 & v_2 & \ldots & v_n\end{pmatrix}$ is the scalar field corresponding to nodes $i \in \{ 1, 2, \ldots, n\}$. The rate of change of $v_i$ is given by

$\frac{dv_i}{dt} = \sum_{j=1}^{n}A_{ji}(v_j - v_i)$

# Modeling Network Diffusion
Can we cast this model in terms of some node and edge functions? In this case
the nodes do not have internal dynamics -- when disconnected from the rest of the
networks, the scalar field at the node becomes completely static.

Neighbour-neighbour interactions in `NetworkDynamics.jl` are described by equations for the edges.

	Hmmm but how do we then implement the fungi-mediated dynamics?

Don't know that yet. Let's stick with this for now and see if we can figure something out from the docs.
"

# ╔═╡ 18628a41-9a11-4272-9d14-41d82c36659e
begin
"""
- `e`: current state of the edge
- `v_s`: source vectex
- `v_d`: destination vertex

This function is then passed to a `StaticEdge` constructor.
The edge's value `e` equals the difference of the terminal vertices' scalars.
"""
...
# ╔═╡ 954250f3-1f87-4b00-bb51-c9e3523166ed
begin
"""
This is passed to an edge constructor. `dv` is the derivative of the vertex state.
For undirected graphs, the `edgefunction!` specifies the coupling from a source to aa destination vertex. The contributions of the connected edges are added to the destination at each edge. 
"""
...
# ╔═╡ d345b3ee-22b2-4f50-8e69-ddab80b63e5a
md"
In essence, this method of constructing the network stores the gradient in the edge and then accumulates values stored in the edge in to the node."

# ╔═╡ 0f062145-e389-4dc1-88f8-b05b31dfe2ad
begin
	N = 20
	k = 4
...
# ╔═╡ 7ca08707-a4d0-4080-a26e-7eebd295d4ef
begin
...
# ╔═╡ 59c6517e-f65a-4f5d-852e-5195748b8293
begin
	x0 = randn(N)
...
# ╔═╡ a9ce501f-c22b-413e-ba99-4acb19efa789
plot(sol; vars=syms_containing(nd, "v"))

# ╔═╡ bcbd8b78-487f-4e73-a370-81330fb5f480
md"
# Independent Diffusions
Let's go
"

# ╔═╡ 3b62eec5-c023-48cf-829c-e70a33436c71
begin
	N₂ = 10
	k₂ = 4
	g₂ = barabasi_albert(N₂, k₂)
...
# ╔═╡ 0e729d83-4d23-47cb-94f8-ec8cc273b4a3
begin
...
Graphs = "~1.7.0"
LaTeXStrings = "~1.3.0"
NetworkDynamics = "~0.7.1"
OrdinaryDiffEq = "~6.16.1"
Plots = "~1.30.1"
"""
...
git-tree-sha1 = "394abe62b8cc045c2d94981acf83edb946c5c76b"
...
[[deps.NetworkDynamics]]
deps = ["Graphs", "LinearAlgebra", "NLsolve", "SciMLBase", "SparseArrays"]
git-tree-sha1 = "75a990c89976cc00ef8e4ce2c1a4b8468cf75be2"
...
git-tree-sha1 = "46735484f86ca157cc71ae0411b9d1665df57096"
